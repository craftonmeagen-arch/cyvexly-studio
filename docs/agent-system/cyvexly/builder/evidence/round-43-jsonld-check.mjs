...
async function checkRoute(url) {
    ...
    expression: `
      (() => {
        ...
        try {
          ...
          return { found: true, type: obj["@type"], name: obj.name, phone: obj.contactPoint[0].telephone, email: obj.contactPoint[0].email };
        } catch (e) {
  ...
  return { url, consoleMessages, networkFailures, jsonLd: evalResult.result.value };
}

const routes = ["http://localhost:5173/", "http://localhost:5173/contact", "http://localhost:5173/about", "http://localhost:5173/services/business-websites"];
