# Cyvexly Build Team Assignment — Cyvexly Studio

Canonical team: **Cyvexly Build Team**. Active product: Cyvexly Studio at
`C:/app projects/website` on deployment branch `main`.

Owner direction `2026-09-13-05` makes **Chunk 13 — EduAILenz/Mudoinkle
Showcase Parity** the primary Builder assignment. Local candidate `121918d`
replaces generic marketing-tour evidence with three verified, buyer-readable
task journeys and paired desktop/phone proof for each product. EduAILenz assets
hash-match accepted Team 2 source `9e48f41d`, use an empty setup or synthetic
test account, and contain no real student data. Mudoinkle assets were captured
from three separately exercised live public staging preview states.

The next product action is exact-source independent review of `121918d`, not
publication. Accepted/deployed source remains `2923ce5`. After a clean review,
push the candidate, verify the matching Render deployment and canonical buyer/
search contracts, and complete the Owner-required final visible comparison.

Production indexing, Search Console verification/sitemap submission, GA4,
Cyvexly LLC identity, and the final legal baseline were completed or confirmed
under directions `2026-09-12-01` through `2026-09-12-03`. Stripe remains
deferred for Owner information; Guardio review remains account-side work.

The primary Builder owns only the Cyvexly-facing case-study surfaces. Underlying
EduAILenz/Mudoinkle apps, their repositories, credentials, infrastructure, and
HoneyHearted remain Team 2 scope. Search/profile monitoring continues without
displacing Chunk 13. Mobile-app website integration remains deferred, and no
scheduler change beyond the Owner's existing Builder resumption is authorized.
