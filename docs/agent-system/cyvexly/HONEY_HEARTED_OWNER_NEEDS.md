...
- Production release: domain/hosting choice, analytics/search ownership or a
  no-analytics decision, final visual/content acceptance, and indexing approval.
  **Current safe behavior:** `/honey-hearted` returns meta and response-header
  no-index protection and makes no external requests. **Activation:** complete
  the launch checklist and independent review before removing no-index.
